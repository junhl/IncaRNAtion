...
diversity.
Two useful measures are the positional entropy and the sequence identity. %base pair entropy.
...
The positional entropy is defined over a set $\mathcal{S}$ of sequences of equal length as follows:
\begin{align}
  \sum_{s^1,s^2 \in\mathcal{S}\times\mathcal{S}}\left(\frac{1}{|s^1|}{\sum_{\substack{i\\s^1_i\equiv s^2_i}}1}\right) &&& \text{Seq. identity}
\end{align}
where $s_i$ is the nucleotide at position $i$ in sequence $s$. The sequence identity is the ratio of perfectly conserved positions over
all pairwise sequences in the set. It reflects how over all the sequences the same positions are conserved. 
