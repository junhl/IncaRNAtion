...
\begin{figure*}[ht!]	
	...
	of sampled sequences folding into the target when using only \ourprog. The second shows 
  after processing previous results with \RNAinverse.
	}
...
per structure with classical softwares who can not control the \GC content.%with \RNAinverse. They yield for most parameters
%a high entropy.
%Table.~\ref{tab:nb_rnainv} contains the number of samples with a given \GCContent generated
with  \RNAinverse, \INFORNA, \NUPACK and \frankenstein.  We present in Fig.~\ref{fig:gcdist} those distributions. They have a highly concentrated distribution of \GC content showing one of theirs major deficiencies, with no sequence below $30\%$\GC. Therefore the need to be able to explicitly
...
diversity and stable secondary structure. 
Two useful measures are the sequence identity Boltzmann frequency.
...
The sequence identity is defined over a set $\mathcal{S}$ of sequences of equal length as follows:
\begin{align}
  \sum_{s^1,s^2 \in\mathcal{S}\times\mathcal{S}}\left(\frac{1}{|s^1|}{\sum_{\substack{i\\s^1_i\equiv s^2_i}}1}\right) &&& \text{Seq. identity}
\end{align}
where $s_i$ is the nucleotide at position $i$ in sequence $s$.
Intuitively, this measure captures the variability of sequences generated by a given method.
The Boltzmann frequency is defined, for a structure $\Struct$ and a sequence $s$ as:
\begin{align}
  e^{\frac{-\ES(s,\Struct)}{RT}}/\mathcal{Z}^s  &&&\text{Frequency}
\end{align}
where $\mathcal{Z}^s$ is the partition function of sequence $s$. This measure tells us how dominant is structure $\Struct$ in the Boltzmann ensemble of sequence $s$. A high value implies a 
stable structure. To compute the frequency, we used \rnafold from the \texttt{Vienna Package 2.0} developed by~\cite{Hofacker:1994}.
...
solutions as their average entropy and positions inside base pairs entropy, since 
...
It should be noted, however, than \RNAinverse produces sequences whose \GCContent is extremely concentrated (see Fig.), thereby requiring a large number of runs before producing a suitable \GCContent sequence. Therefore, a rejection algorithm based on \RNAinverse alone should not be considered a realistic solution for the design with constrained \GCContent. 
